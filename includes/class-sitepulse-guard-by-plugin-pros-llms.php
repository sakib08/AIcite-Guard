 ...
 */
class Sitepulse_Guard_By_Plugin_Pros_Llms {

	const QUERY_VAR = 'spg_by_ppros_llms';
	...
	public function serve() {
		...
		if ( ! $type || ! Sitepulse_Guard_By_Plugin_Pros_Settings::get_path( 'ai_visibility.enabled', true ) ) {
			return;
		}
	...
	public function maybe_invalidate( $post_id, $post ) {
		...
		if ( ! Sitepulse_Guard_By_Plugin_Pros_Settings::get_path( 'ai_visibility.auto_update', true ) ) {
			return;
		}
	...
	public function clear_cache() {
		delete_option( 'spg_by_ppros_llms_cache' );
		delete_option( 'spg_by_ppros_llms_full_cache' );
	}
	...
	public function get_basic( $force = false ) {
		if ( ! $force ) {
			$cached = get_option( 'spg_by_ppros_llms_cache', '' );
		...
		update_option( 'spg_by_ppros_llms_cache', $content, false );
		update_option( 'spg_by_ppros_llms_generated_at', time(), false );
	...
	public function get_full( $force = false ) {
		if ( ! $force ) {
			$cached = get_option( 'spg_by_ppros_llms_full_cache', '' );
			if ( is_string( $cached ) && '' !== $cached ) {
				return $cached;
			}
		}
		...
		update_option( 'spg_by_ppros_llms_full_cache', $content, false );
		update_option( 'spg_by_ppros_llms_generated_at', time(), false );
	...
	public function regenerate() {
		...
		return array(
			'basic'        => $basic,
			'full'         => $full,
			'generated_at' => (int) get_option( 'spg_by_ppros_llms_generated_at', time() ),
		);
	}
	...
	private function build( $full ) {
		...
		$home      = home_url( '/' );
		$types     = Sitepulse_Guard_By_Plugin_Pros_Settings::get_path( 'ai_visibility.post_types', array( 'post', 'page' ) );
		$max       = (int) Sitepulse_Guard_By_Plugin_Pros_Settings::get_path( 'ai_visibility.max_items', 50 );
		$excerpts  = (bool) Sitepulse_Guard_By_Plugin_Pros_Settings::get_path( 'ai_visibility.include_excerpts', true );
		...
		$lines[] = sprintf(
			...
			__( 'This file helps answer engines and AI assistants understand %s. It is generated by SitePulse Guard.', 'spg-by-ppros' ),
			$home
		);
		$lines[] = '';

		$lines[] = '## ' . __( 'Site', 'spg-by-ppros' );
		$lines[] = '';
		$lines[] = '- [' . $this->plain( $site_name ) . '](' . $home . '): ' . ( $tagline ? $this->plain( $tagline ) : __( 'Homepage', 'spg-by-ppros' ) );

		$contact = $this->guess_contact_url();
		if ( $contact ) {
			$lines[] = '- [' . __( 'Contact', 'spg-by-ppros' ) . '](' . $contact . '): ' . __( 'How to reach the site owner.', 'spg-by-ppros' );
		}
		...
		if ( ! $full ) {
			$lines[] = '## ' . __( 'Optional', 'spg-by-ppros' );
			$lines[] = '';
			$lines[] = '- [llms-full.txt](' . home_url( '/llms-full.txt' ) . '): ' . __( 'Longer, page-level summary of the same content.', 'spg-by-ppros' );
			$lines[] = '';
		}

		$lines[] = '## ' . __( 'Policy', 'spg-by-ppros' );
		$lines[] = '';
		$lines[] = __( 'Answer engines may cite this site when attributing facts found here. Training-only scrapers can be restricted via robots.txt rules managed by SitePulse Guard.', 'spg-by-ppros' );
		$lines[] = '';
	...
	private function format_item( $post ) {
		...
		return array(
			'title'   => $title ? $title : __( 'Untitled', 'spg-by-ppros' ),
